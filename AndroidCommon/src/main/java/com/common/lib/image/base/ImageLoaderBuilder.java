...
public class ImageLoaderBuilder implements IImageLoader {
    ...
    public void load(ImageView imageView) {
        ...
        if (isGif) {
            ...
            if (context != null) {
                iBaseImageLoader.loadGifImage(context, type, imageView, url, isAnim, iterations, blurRadius);
            } else if (fragment != null) {
                iBaseImageLoader.loadGifImage(fragment, type, imageView, url, isAnim, iterations, blurRadius);
            }
            return;
        }
        // 普通图加载
